    ...
    <?php if (!empty($banners)): ?>
        <?php foreach ($banners as $i => $banner): ?>
            ...
            <?php if ($tipo === 'imagem_link'): ?>
                ...
                <a href="<?= sanitize($banner['link'] ?? '#') ?>" class="hero-slide <?= $i === 0 ? 'active' : '' ?>" target="_blank" style="cursor: pointer;" data-duration="<?= (int)($banner['duracao'] ?? 5000) ?>">
                    <img src="<?= url($banner['imagem']) ?>" alt="<?= sanitize($banner['titulo'] ?? 'Banner') ?>">
                </a>

            <?php elseif ($tipo === 'imagem_texto'): ?>
                <!-- Tipo: imagem + texto + botão -->
                <div class="hero-slide <?= $i === 0 ? 'active' : '' ?>" data-duration="<?= (int)($banner['duracao'] ?? 5000) ?>">
            ...
            <?php else: ?>
                ...
                <div class="hero-slide <?= $i === 0 ? 'active' : '' ?>" style="background: <?= sanitize($banner['cor_fundo'] ?? '#1a1a1a') ?>;" data-duration="<?= (int)($banner['duracao'] ?? 5000) ?>">
                    <div class="container">
                        <div class="hero-content">
                            <?php if ($banner['titulo']): ?>
                                <h2><?= $banner['titulo'] ?></h2>
